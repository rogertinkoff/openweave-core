...
WEAVE_ERROR LocaleSettingsTraitUpdatableDataSink::SetProperty(SubscriptionClient * apSubClient,
                                                         bool aIsConditional, const char * locale)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
    ...
    MOCK_strlcpy(mLocale, locale, sizeof(mLocale));
...
exit:
    WeaveLogDetail(DataManagement, "LocaleSettingsTrait with error %d", err);
